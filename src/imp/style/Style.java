...
public class Style
        implements Constants, Serializable
  {
    ...
    private Polylist interpolations = new Polylist();
    ...
    /**
     *
     * @param voicingFileName voicing file name to search for in voicing
     *                        directory
     */
                    ...
                    switch( Leadsheet.lookup((String) dispatcher, keyword) )
                      {
                        ...
                        case CHORD_PATTERN:
                          {
                            ...
                            cp.setStyle(style);
                            cp.setDefinedRules(cp.getStyle().
                                    getChordDefinedRules());
                            cp.makePattern(item);
                        ...
                        case DRUM_PATTERN:
                          {
                            ...
                            dp.setStyle(style);
                            dp.setDefinedRules(dp.getStyle().
                                    getDrumDefinedRules());
                            dp.makePattern(item);
                        ...
                        case BASS_PATTERN:
                          {
                            ...
                            bp.setStyle(style);
                            bp.setDefinedRules(bp.getStyle().
                                    getBassDefinedRules());
                            bp.makePattern(item);
                        ...
                        case COMMENTS:
                          {
                            String commentsString = Leadsheet.concatElements(
                                    item);
                        ...
                        case INTERPOLATE:
                          {
                            style.interpolations = style.interpolations.cons(
                                    Interpolant.makeInterpolantFromExp(item));
            ...
            case CHORD_BASE:
              {
                ...
                while( chord.hasMoreElements() )
                  {
                    NoteSymbol note
                            = NoteSymbol.makeNoteSymbol((String) chord.
                                    nextElement());
     ...
     *
     * @param L e.g. L is (drum name (rules X4 R4 X4 R4))
     */
    ...
    public void saveLeadsheet(BufferedWriter out) throws IOException
    {
        ...
        out.newLine();
        out.write(
                "    (" + keyword[BASS_HIGH] + " " + bassHigh.toPitchString() + ")");
        out.newLine();
        out.write(
                "    (" + keyword[BASS_LOW] + " " + bassLow.toPitchString() + ")");
        out.newLine();
        out.write("    (" + keyword[CHORD_HIGH] + " " + chordHigh.
                toPitchString() + ")");
        out.newLine();
        out.write(
                "    (" + keyword[CHORD_LOW] + " " + chordLow.toPitchString() + ")");
        out.newLine();
        out.write(
                "    (" + keyword[VOICING_FILE] + " " + this.voicingFileName + ")");
        out.newLine();
        out.write("    " + NoteSymbol.makePitchStringList(chordBase).cons(
                keyword[CHORD_BASE]));
        out.newLine();
...
// Sum the weights of the patterns.
        ...
        System.out.println(
                "no ChordPattern of duration " + desiredDuration + " found");
        return null;
    }
    ...
    public static boolean limitNotReached(long time, int endLimitIndex)
    {
        return true // ??? This will ALWAYS return true
                || endLimitIndex == ENDSCORE // i.e. play to end
     ...
     * @param currentChord  a ChordSymbol containing the currentChord
     *                      currentChord to render
    ...
    // we manually render polylists for each PC this method.
          ...
          // A pattern can contain volume information.
            ...
            if( pattern == null || constantBass )
              ...
              //System.out.println("pattern == null case in Style");
                ...
                Polylist v = ChordPattern.findVoicing(symbol, previousChord,
                                                      this);
                        ...
                        if( ob instanceof NoteSymbol )
                          {
                            ...
                            note.render(ms, seq.getChordTrack(), time, offTime,
...
//System.out.println("MIDI sequence = " + MidiFormatting.prettyFormat(sequence2polylist(seq)));
     ...
     *
     * @param bassline         a LinkedList of NoteSymbols making up the
     *                         bassline so far
     * @param chord            a ChordSymbol containing the currentChord chord
     *                         to render
            ...
            if( pattern == null || constantBass )
              {
                ...
                NoteSymbol bassNote = new NoteSymbol(bassPitchClass, octave,
                                                     duration);
...
// Find the last non-rest in the segment.
                ...
                if( lastOb instanceof Polylist && basslineSegment.getFirst() instanceof NoteSymbol )
                  {
                    ...
                    int pDur = Duration.getDuration0(dur) + ns.toNote().
                            getRhythmValue();
    ...
    {
        double sum = 0;
        Polylist defaultchords = Polylist.list(interpolations.first(),
                                               interpolations.second());
...
// ??? Both branches of the following do the same thing
            if( pc.getRoot().equals("g") )
              {
                pc.transpose(8 + PitchClass.findRiseToC(c.getRoot()));
              }
            else
              {
                pc.transpose(8 + PitchClass.findRiseToC(c.getRoot()));
        ...
        for( Polylist P = polations; P.nonEmpty(); P = P.rest() )
          {
            if( ((Interpolant) P.first()).getTARGETS().member("C" + c.
                    getQuality()) )
              {
...
// ???? Both branches of the following do the same thing
        ...
        // Why not just return pchords.nth(nth)??
        int nth = 0;
        for( Polylist A = pchords; A.nonEmpty(); A = A.rest() )
          {
            if( nth == random )
              {
                return (Chord) A.first();
              }
            else
              {
                nth++;
     ...
     *
     * Using the Pattern objects of this Style, sequences an accompaniment for
     * the
     * given ChordPart.
     *
     * @param seq       the Sequence that contains the Track
    ...
    {
        if( list.isEmpty() || (boolean) list.first() )
          {
            return Polylist.list((start + 1), list.rest());
          }
        else
          {
            return findCorrectDuration(list.rest(), start++);
          }
    }
...
// + " endIndex = " + endIndex + " endLimitIndex = " + endLimitIndex + " useDrums = " + useDrums + " hasStyle = " + hasStyle);
        ...
        NoteSymbol previousBassNote
                = NoteSymbol.makeNoteSymbol((bassHigh.getMIDI() + bassLow.
                        getMIDI()) / 2);
        Polylist previousChord = Polylist.nil;

        int blockIndex = 0;
        // Polylist finder = Interpolate.ArraytoPoly(chordPart.getBlock(blockIndex).flattenBlock());
            ...
            if( !chord.isNOCHORD() && hasStyle )
              {
                ...
                if( chordPart.getChord(index) != null && interpolations.
                        nonEmpty() )
                  {
                    Chord newChord = null; // NOT WORKING!!! was getPC(chordPart.getChord(index));

                    Polylist finder = Interpolate.ArraytoPoly(chordPart.getBlock(blockIndex).flattenBlock());
                    boolean correctBrick = false;
                    for( Polylist F = finder; F.nonEmpty(); F = F.rest() )
                      {
                        if( ((ChordBlock) F.first()).getName().equals(
                                currentChord.getName()) )
                          {
                            correctBrick = true;
                            break;
                          }
                    ...
                    while( !correctBrick )
                      {
                        blockIndex++;
                        finder = Interpolate.ArraytoPoly(chordPart.getBlock(blockIndex).flattenBlock());
                        for( Polylist F = finder; F.nonEmpty(); F = F.rest() )
                          {
                            if( ((ChordBlock) F.first()).getName().equals(currentChord.getName()) )
                              {
                                correctBrick = true;
                                break;
                              }
                    ...
                    //  System.out.println("currentChord used for if statement");
                    Polylist willInterp = Polylist.nil; //NOT WORKING!! Interpolate.willInterpolate(chordPart.getBlock(blockIndex), currentChord);
                    if( willInterp.nonEmpty() && willInterp.member(true) )
                      {
                      // System.out.println("current chord: " + currentChord + " and time: " + time/4);
                      //                    System.out.println("willInterpolate() stuff: " + Interpolate.willInterpolate(
                      //                        chordPart.getBlock(blockIndex),currentChord)
                      //                        + "   " + currentChord + "   " + chordPart.getBlock(blockIndex) + "\n");    
                        while( willInterp.nonEmpty() && willInterp.member(true) )
                          {
                            // System.out.println("results from correct duration: " + findCorrectDuration(willInterp,1));
                            int minVal = ((Interpolant) interpolations.first()).getMINSLOTS();
                            int originalRV = currentChord.getRhythmValue();
                            currentChord.setRhythmValue(
                                    minVal * (int) findCorrectDuration(
                                            willInterp, minVal / 120).first());
                            newChord.setRhythmValue(
                                    originalRV - minVal * (int) findCorrectDuration(
                                            willInterp, minVal / 120).first());
                            //currentChord.setRhythmValue(currentChord.getRhythmValue()/2);
                            //newChord.setRhythmValue(currentChord.getRhythmValue());
                            System.out.println("bar " + (index / 480) + " current chord going into previous chord: " + currentChord);
                            ...
                            System.out.println("bar " + (index / 480) + " interpolated chord: " + newChord);
                            previousChord = makeChordline(seq,
                                                          time,
                                                          newChord,
                                                          previousChord,
                            ...
                            newChord = getPC(newChord);
                            willInterp = (Polylist) findCorrectDuration(willInterp, 1).last();
                          }
                      }
                    else
                      {
                        //System.out.println("bar " + (index / 480) + " current chord outside if: " + currentChord);
                        previousChord = makeChordline(seq,
                                                      time,
                                                      currentChord,
                                                      previousChord,
                  ...
                  {
                    //System.out.println("bar " + (index / 480) + " current chord: " + currentChord + " and time " + time / 4);
                    Polylist finder = Interpolate.ArraytoPoly(chordPart.getBlock(blockIndex).flattenBlock());
                    boolean correctBrick = false;
                    for( Polylist F = finder; F.nonEmpty(); F = F.rest() )
                      {
                    ...
                    while( !correctBrick )
                      {
                        blockIndex++;
                        finder = Interpolate.ArraytoPoly(chordPart.getBlock(
                                blockIndex).flattenBlock());
                        for( Polylist F = finder; F.nonEmpty(); F = F.rest() )
                          {
...
//                    System.out.println(
//                            "willInterpolate() stuff: " + Interpolate.
//                                    willInterpolate(
//                                            chordPart.getBlock(blockIndex),
//                                            currentChord)
//                            + "   " + currentChord + "   " + chordPart.getBlock(
//                                    blockIndex));
...
// Sets previousBassNote to last NoteSymbol in bassline
                    for( Iterator<MelodySymbol> it = bassline.
